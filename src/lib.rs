...
fn init(mut exports: JsObject) -> Result<()> {
  ...
  exports.create_named_method("rust_fb", rust_fb)?;
...
fn rust_f(mut n: u32) -> u32 {
  if n<=2 {
    return n
  }
  return rust_f(n-1)+rust_f(n-2)
}

#[js_function(1)]
fn rust_fb(ctx: CallContext) -> Result<JsNumber> {
  let argument: u32 = ctx.get::<JsNumber>(0)?.try_into()?;
  let res = rust_f(argument);
